...
table_id_target = 'stock-410113.stock_google.prediction_table'
...
def extract_data(name, client):
    sql = """
        WITH latest_date AS (
        SELECT MAX(ny_data_date) as max_date
        FROM `stock-410113.stock_google.new_table`
        )

        SELECT *
        FROM stock-410113.stock_google.new_table
        WHERE ny_data_date = (SELECT max_date FROM latest_date)
            AND company_name = "{0}"
    """.format(name)
...
def sarima_forecast(model, df, forecast_date, scaler):
    ...
    df_result = pd.concat([df, fitted_series, lower_series, upper_series], axis=1, join= 'outer')
...
def load_weight_model(data_transformed):
    ...
    lstm_model.load_weights("/Users/susanoo/Desktop/MSBA/career/github_projects/best_weights.h5")
...
def PredictStockPriceLSTM(Model , df , ForecastDate , scaler, feature_length = 20):
    ...
    df = pd.DataFrame(np.squeeze(scaler.inverse_transform(df)), index=df.index, columns=['close'])
    return df
...
def combine_model(data_transformed, lstm_model, sarima_model, forecast_date, scaler):
  ...
  for i in range((datetime.strptime(forecast_date, '%Y-%m-%d') - datetime.combine(data_transformed.index[-1], datetime.min.time())).days):
...
def forecast_price(name, forecast_date, client):
    ...
    scaler = load_object("/Users/susanoo/Desktop/MSBA/career/github_projects/Scaler.pkl")
    sarima_model = load_object("/Users/susanoo/Desktop/MSBA/career/github_projects/sarima_model.pkl")
    ...
    df_result['ny_data_date'] = datetime.now(pytz.timezone("America/New_York")).date().strftime('%Y-%m-%d')
    df_result["ny_data_date"] = df_result["ny_data_date"].apply(pd.to_datetime)
    ...
    columns_order = ['ny_data_date', 'company_name', 'Prediction', 'High', 'Low', 'forecast_date']
...
with DAG(dag_id='forecast_stock_price',
    ...
    forecast_stock_price = forecast_price("GOOG", "2024-01-14", client)
